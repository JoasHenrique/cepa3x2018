        ...
        this.verifyClient = (info, next) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (req.roomId) {
                try {
                    ...
                    const hasReservedSeat = yield this.matchMaker.remoteRoomCall(req.roomId, 'hasReservedSeat', [query.sessionId], 300);
                    if (!hasReservedSeat) {
                        const isLocked = yield this.matchMaker.remoteRoomCall(req.roomId, 'locked', undefined, 300);
                        if (isLocked) {
                            return next(false, Protocol_1.Protocol.WS_TOO_MANY_CLIENTS, 'maxClients reached.');
                        }
                    }
                    // verify client from room scope.
                    const authResult = yield this.matchMaker.remoteRoomCall(req.roomId, 'onAuth', [req.options]);
                    if (authResult) {
                        req.auth = authResult;
                        next(true);
                    }
